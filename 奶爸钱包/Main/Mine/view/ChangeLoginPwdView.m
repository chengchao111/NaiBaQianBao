...
@implementation ChangeLoginPwdView
...
-(void)setUpUI{
    ...
    NSArray *titleArr = @[@"当前密码",@"登录密码",@"确认密码"];
    for (int i = 0; i < titleArr.count; i ++) {
        ...
        label.frame = CGRectMake(SCREENW(11), SCREENH(32) + SCREENH(60) * i, SCREENW(78), SCREENH(43));
    ...
    UILabel *label1 = [ControlUtiles createText:@"登录密码由6-15位数字或字母组合" textColor:[UIColor blackColor] backgroundColor:[UIColor clearColor] font:SCREENW(14) textAlignment:(NSTextAlignmentCenter)];
...
-(UITextField *)oldPwdTf{
    if (!_oldPwdTf) {
        _oldPwdTf = [ControlUtiles creatTextFiledWithTextColor:[UIColor blackColor] font:SCREENW(15) placeHolder:@"输入当前密码"];
    }
    return _oldPwdTf;
}
...
-(UITextField *)nowPwdTf{
    if (!_nowPwdTf) {
        _nowPwdTf = [ControlUtiles creatTextFiledWithTextColor:[UIColor blackColor] font:SCREENW(15) placeHolder:@"登录密码为6-15位字母或数字组合"];
    }
    return _nowPwdTf;
}
...
-(UITextField *)sureNowPwdTf{
    if (!_sureNowPwdTf) {
        _sureNowPwdTf = [ControlUtiles creatTextFiledWithTextColor:[UIColor blackColor] font:SCREENW(15) placeHolder:@"请输入确认登录密码"];
    }
